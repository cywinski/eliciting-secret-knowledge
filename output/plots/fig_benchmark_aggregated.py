# ABOUTME: Aggregated benchmark plot showing Taboo, SSC, and User Gender results.
# ABOUTME: Taboo results are averaged across models (Gemma, Llama). Empty bars for missing data.
# %%
import json
...
# Taboo models to average over
TABOO_MODELS = ["gemma"]
TABOO_SECRETS = ["flag", "gold", "moon"]

# Gender models and genders to average over
GENDER_MODELS = ["gemma"]
GENDERS = ["female", "male"]
...
def get_ssc_method_results(
    ...
    prefix = "results_base_test_llama" if is_base else "results_test_llama"
    result_file = (
        SSC_RESULTS_DIR
        / prefix
        / prompt_type
    ...
    if not result_file.exists():
        # For I/O baseline, use finetuned results (same for both base and finetuned)
        if is_base and method_dir == "io":
            fallback_file = (
                SSC_RESULTS_DIR
                / "results_test_llama"
                / prompt_type
            ...
            if fallback_file.exists():
                print(f"  [SSC] Loading (fallback): {fallback_file}")
                return load_gpt_metrics(fallback_file)
        return None, None
...
def create_plot(
    ...
    output_file: str = "fig_benchmark_aggregated.pdf",
):
    ...
    for col_idx, data in enumerate(datasets):
        ...
        if col_idx == 0:
            ax.set_title("Secret-keeping model", fontsize=TITLE_FONT_SIZE)
        elif col_idx == 1:
            ax.set_title("Base model", fontsize=TITLE_FONT_SIZE)
...
if __name__ == "__main__":
    ...
    print_data_summary(data_finetuned, "Secret-keeping model")
    print_data_summary(data_base, "Base model")

    output_file = output_dir / f"fig_benchmark_aggregated_{prompt_type}.pdf"
