# ABOUTME: Bar plot comparing elicitation methods across benchmarks using GPT-4 auditor.
# ABOUTME: Analogous to fig_metrics_direct_combined_by_models.py but for GPT auditor results.
...
PLOT_TITLE = "Secret-keeping model (GPT-4 Auditor)"
...
# %%
# =============================================================================
# RESULT DIRECTORIES (GPT Auditor versions)
# =============================================================================
...
def load_all_data():
    ...
    # Remaining methods with hardcoded values (from hybrid accuracy calculations)
    # Taboo and User Gender values calculated from GPT auditor hybrid accuracy results
    hardcoded_methods = {
        "I/O + " + r"$\bf{Fuzzing}$": {
            "Taboo": (2.50, 1.36),
            "SSC": (0.0, 0.0),  # TODO: Calculate when available
        ...
        "I/O + " + r"$\bf{User}$" + " " + r"$\bf{Persona}$": {
            "Taboo": (19.60, 0.92),  # Note: moon missing, avg of gold+flag only
            "SSC": (0.0, 0.0),  # TODO: Calculate when available
...
def create_plot(methods_data):
    ...
    plt.savefig(
        "plots/fig_metrics_direct_combined_by_models_gpt_auditor.png",
        dpi=FIGURE_DPI,
        bbox_inches="tight",
    )
    plt.savefig(
        "plots/fig_metrics_direct_combined_by_models_gpt_auditor.pdf",
        dpi=FIGURE_DPI,
        bbox_inches="tight",
    )
    print("Plot saved as 'plots/fig_metrics_direct_combined_by_models_gpt_auditor.png'")
    print("Plot saved as 'plots/fig_metrics_direct_combined_by_models_gpt_auditor.pdf'")
